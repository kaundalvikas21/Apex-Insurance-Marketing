...
- **CTA weighting:** phone first. The hero's one button is `.btn-call.btn-xl`, above the fold,
  repeated in a navy `call_band()` after sections 4, 6, and 8. The four-field form is the secondary
  panel, in the contact section under the strip. Both of the hub's forms are `callback_form()` calls now; the two hand-rolled copies are gone. The hero is the full-bleed banner `fe-hero`.
...
- Layout families in order: hero, strip, split + panel, card + scale, table, split + card, navy band,
