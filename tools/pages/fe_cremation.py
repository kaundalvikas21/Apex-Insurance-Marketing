...
DESC = ("A cremation costs less than a burial, so the policy that covers it is smaller. How to "
        "size one, and how to get a real local price in two phone calls.")

TRAIL = [("Home", "/"), ("Final Expense Insurance", "/final-expense-insurance/"),
         ("Cremation insurance", None)]

SIZING = [
    ("Call two funeral homes or crematories near you",
     "Ask each for its general price list. They are required to give it to you, and you do not "
     "have to explain why you want it. Ask specifically for the direct cremation price and the "
     "price of a cremation with a service. The gap between those two is often larger than people "
     "expect, and it is the main thing that decides your number."),
    ("Add the things the price list leaves out",
     "The urn, if you want a particular one. Death certificates, which are charged per copy, and "
     "you will need several. Transport, if the death happens away from home. Any venue or "
     "catering, if there is a gathering. This is usually a smaller number than the cremation "
     "itself, but it still counts."),
    ("Add a margin for the bills that follow, then stop",
     "Medical balances, the last utility bills, and the filings a death generates. Once you "
     "have that total, buy that much coverage and not more. Rounding up to the next round number "
     "sounds harmless. It is how people end up paying for years for coverage they never "
     "sized."),
]

DIFFERENCE = [
    ("The coverage amount is smaller", "A direct cremation is the least expensive option most "
     "funeral homes offer. A cremation with a service still normally costs well below a "
     "burial with a plot and a headstone. So the policy that covers it is smaller, "
...
FAQ = [
    ...
    ("What if my family chooses a burial instead?",
     ...
     "Tell them that while you are alive rather than leaving them to discover "
...
TELL_THEM = """<h3 class="reveal text-h4">Tell your beneficiary the policy exists</h3>
<p class="mt-3 text-slate">
        ...
        This is by far the most common way final expense coverage fails to do its job.
      </p>
      <h3 class="reveal mt-8 text-h4">Tell them you want a cremation</h3>
<p class="mt-3 text-slate">
        Tell them you want a cremation, too, and that the policy is sized for one. It is an
        uncomfortable conversation, and it takes about four minutes. It is the difference
        between your family knowing what you wanted and guessing at it in the worst week of their
        lives.
      </p>
      <h3 class="reveal mt-8 text-h4">Where the premiums are</h3>
<p class="mt-3 text-slate">
...
{C.page_hero(
    ...
    answer=(
        ...
        'premium is lower. This page helps you avoid buying more coverage than you '
        'need.'),
...
<section class="section">
  <div class="container-ax">
    <div class="max-w-2xl">
      <h2 class="text-h2">How cremation insurance differs from burial insurance</h2>
      <p class="mt-5 text-slate">
        One thing changes. Two things do not, and both are worth knowing before you talk to an
        agent.
      </p>
    </div>
...
{C.steps_section("Size your cremation policy in two phone calls",
         "We do not print a cremation cost figure on this page. Cremation costs vary so much "
...
{C.inline_cta(
    ...
    "your state, and answer a short list of health questions. We will tell you what carriers "
...
{C.prose("Two conversations to have before you buy", TELL_THEM,
         intro="Both are short, and both spare your family from guessing later.",
